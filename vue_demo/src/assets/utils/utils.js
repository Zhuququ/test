export const _debounce = function (func, wait, immediate) {
    ...
    let later = function () {
        ...
        if (last < wait && last >= 0) {
            ...
            // 如果不是立即执行，随即执行 func 方法，回避了immediate=true时，已经执行的问题
            if (!immediate) {
    ...
    return function () {
        context = this // 调用debounce的对象
        args = arguments // 调用debounce的参数
        // 每次触发函数，更新时间戳
        // later 方法中取 last 值时用到该变量
        // 判断距离上次触发事件是否已经过了 wait seconds 了
        // 即我们需要距离最后一次事件，触发 wait seconds 后触发这个回调方法
        ...
        // 因为第一次触发后已经设置了 timeout（line 48），所以根据 timeout 是否为空可以判断是否是首次触发
        let callNow = immediate && !timeout
        
        // 设置 wait seconds 后触发 later 方法
        // 无论是否 callNow（如果是 callNow，也进入 later 方法，去 later 方法中判断是否执行相应回调函数）
        // 在某一段的连续触发中，只会在第一次触发时进入这个 if 分支中，因为只有在结束触发后，timeout 才会重置，否则timeout 一直有值
        if (!timeout) {
...
/**
 * {leading: false}, 程序设定第一个回调不是立即执行
 *
 * {trailing: false}，即最后一次不需要触发
 * 
 */
export const _throttle = function(func, wait, options) {
    ...
    return function() {
      ...
      // 并且如果程序设定第一个回调不是立即执行的话（options.leading === false）
